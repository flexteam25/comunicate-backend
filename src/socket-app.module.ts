...
/** Same entities as AppModule so TypeORM has full metadata (socket server only needs auth/admin for JWT validation). */
const ALL_ENTITIES = [
  User,
  UserOldPassword,
  UserToken,
...
/**
 * Minimal app module for the standalone socket server (port 3009).
 * Uses same TypeORM entities as main API so all relation metadata is available.
 */
@Module({
  imports: [
