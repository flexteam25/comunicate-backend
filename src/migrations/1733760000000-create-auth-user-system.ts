...
export class CreateAuthUserSystem1733760000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex('user_tokens', new TableIndex({ name: 'IDX_user_tokens_user_id', columnNames: ['user_id'] }));
    await queryRunner.createIndex('user_tokens', new TableIndex({ name: 'IDX_user_tokens_token_id', columnNames: ['token_id'] }));
    ...
    await queryRunner.createIndex('user_permissions', new TableIndex({ name: 'IDX_user_permissions_permission_id', columnNames: ['permission_id'] }));
    await queryRunner.createIndex('user_badges', new TableIndex({ name: 'IDX_user_badges_user_id', columnNames: ['user_id'] }));
    await queryRunner.createIndex('user_badges', new TableIndex({ name: 'IDX_user_badges_badge_id', columnNames: ['badge_id'] }));
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (userTokensTable) {
      const fk = userTokensTable.foreignKeys.find((fk) => fk.columnNames.indexOf('user_id') !== -1);
      if (fk) await queryRunner.dropForeignKey('user_tokens', fk);
    }

    if (userRolesTable) {
      const fk1 = userRolesTable.foreignKeys.find((fk) => fk.columnNames.indexOf('user_id') !== -1);
      const fk2 = userRolesTable.foreignKeys.find((fk) => fk.columnNames.indexOf('role_id') !== -1);
    ...
    if (userPermissionsTable) {
      const fk1 = userPermissionsTable.foreignKeys.find((fk) => fk.columnNames.indexOf('user_id') !== -1);
      const fk2 = userPermissionsTable.foreignKeys.find((fk) => fk.columnNames.indexOf('permission_id') !== -1);
      if (fk1) await queryRunner.dropForeignKey('user_permissions', fk1);
      if (fk2) await queryRunner.dropForeignKey('user_permissions', fk2);
    }

    if (userBadgesTable) {
      const fk1 = userBadgesTable.foreignKeys.find((fk) => fk.columnNames.indexOf('user_id') !== -1);
      const fk2 = userBadgesTable.foreignKeys.find((fk) => fk.columnNames.indexOf('badge_id') !== -1);
