...
export class UpdateCommentUseCase {
  constructor(
    ...
    private readonly commentRepository: IScamReportCommentRepository,
    private readonly transactionService: TransactionService,
    private readonly uploadService: UploadService,
  ) {}

  async execute(command: UpdateCommentCommand): Promise<ScamReportComment> {
    ...
    const newImageCount = command.imageUrls?.length || 0;
    ...
    const result = await this.transactionService.executeInTransaction(
      async (manager: EntityManager) => {
        ...
        if (command.imageUrls && command.imageUrls.length > 0) {
          ...
          const imageEntities = command.imageUrls.map((imageUrl, index) =>
            imageRepo.create({
